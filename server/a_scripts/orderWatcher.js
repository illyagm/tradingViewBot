...
(() => {
    ...
    function cancelPendingOrder() {
        const pendingButton = document.querySelectorAll(".oui-tab-trigger.oui-inline-flex")[3];
        ...
        // Click en Pending
        pendingButton.dispatchEvent(new MouseEvent("pointerdown", opts));
        pendingButton.dispatchEvent(new MouseEvent("mousedown", opts));
        pendingButton.dispatchEvent(new MouseEvent("mouseup", opts));
        pendingButton.dispatchEvent(new MouseEvent("click", opts));

        // Delay corto para que el UI reaccione
        setTimeout(() => {
            const cancelButton = document.querySelectorAll(".oui-button.oui-inline-flex.oui-h-7.oui-text-2xs")[0];
            if (!cancelButton) return;

            cancelButton.dispatchEvent(new MouseEvent("pointerdown", opts));
            cancelButton.dispatchEvent(new MouseEvent("mousedown", opts));
            cancelButton.dispatchEvent(new MouseEvent("mouseup", opts));
            cancelButton.dispatchEvent(new MouseEvent("click", opts));
        }, 250);
        emit("order_canceled");
    }
